  ...
  */
  $.getJSON('http://valueinvestingtool.com/practice-linda/biographical.json', function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      if (nameOfSector == data[i]['sector'] && !(nameOfSector in matchingSectors)) { // if sector names match and not already in a list of cells
    ...
    $('a.aIndustry').click( function(e) {
      ...
      */
      $.getJSON('http://valueinvestingtool.com/practice-linda/biographical.json', function(data) {
        ...
        //
        for (var i = 0; i < data.length; i++) {
          if (nameOfIndustry == data[i]['industry'] && !(nameOfIndustry in matchingIndustries)) { // if industry names match and not already in a list of cells
            matchingIndustries.push(data[i]);
          }
        }
        ...
        $.getJSON('http://valueinvestingtool.com/practice-linda/technical.json', function(data) {
          ...
          for (var x = 0; x < numListOfTickers; x++) {
            for (var y = 0; y < data.length; y++) {
              if (listOfTickers[x] == data[y]['ticker']) {
                matchingTickers.push(data[y]);
              }
            }
          }
